            ...
            get(target, prop) {
                ...
                if (paramIndex != -1) {
                    ...
                    if(_wl_material_get_param_value(target._index, paramIndex, WL._tempMem)) {
                        if(paramType.type == 0) {
                            return type.componentCount == 1 ? WL._tempMemUint32[0] : new Uint32Array(HEAPF32.buffer, WL._tempMem, paramType.componentCount);
                        }
                        if(paramType.type == 1) {
                            return type.componentCount == 1 ? WL._tempMemInt[0] : new Int32Array(HEAPF32.buffer, WL._tempMem, paramType.componentCount);
                        }
                        if(paramType.type == 2) {
                            return type.componentCount == 1 ? WL._tempMemFloat[0] : new Float32Array(HEAPF32.buffer, WL._tempMem, paramType.componentCount);
                        }
                        if(paramType.type == 3) {
